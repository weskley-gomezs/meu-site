      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-display font-bold text-brand-dark mb-4">
            INVESTIMENTO PARA O SEU <span className="text-brand-purple">CRESCIMENTO</span>
          </h2>
          ...
          {services.map((service, idx) => (
            ...
            >
              ...
              <div className={`w-16 h-16 rounded-xl flex items-center justify-center mb-6 transition-colors duration-300 ${
                ...
                : 'bg-gray-50 text-brand-dark group-hover:bg-brand-purple group-hover:text-white'
              }`}>
                {service.icon}
              </div>
              
              <h3 className="text-2xl font-display font-bold text-gray-900">{service.title}</h3>
              <p className="text-gray-500 mb-6 font-medium">{service.subtitle}</p>
              ...
              <ul className="space-y-4 mb-8">
                {service.features.map((feature, i) => (
                  <li key={i} className="flex items-center text-gray-600">
                    <CheckCircle2 size={18} className={`mr-3 flex-shrink-0 transition-colors ${service.highlight ? 'text-green-500' : 'text-gray-400 group-hover:text-green-500'}`} />
                    {feature}
                  </li>
                ))}
              </ul>
              ...
              <a 
                ...
                className={`
                  ...
                  ${service.highlight 
                    ...
                    : 'bg-gray-50 text-brand-dark border border-gray-200 hover:bg-white hover:border-brand-purple hover:text-brand-purple hover:shadow-lg hover:-translate-y-1'
                  }
                `}
              >
